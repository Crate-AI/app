...
import { useAuthStore, useTracksStore, usePlaylistStore, usePlayerStore } from '@/stores';
...
const DashboardStats = ({ tracks }: { tracks: CrateTrack[] }) => {
  const stats = {
    ...
    avgBpm: tracks.filter(t => t.bpm).length > 0 
      ? Math.round(tracks.filter(t => t.bpm).reduce((acc, t) => acc + (t.bpm || 0), 0) / tracks.filter(t => t.bpm).length)
      : 0,
    totalArtists: new Set(tracks.map(t => t.artist)).size,
  };

  const statItems = [
    { label: 'Total Tracks', value: stats.totalTracks, icon: Music, color: 'bg-blue-100 text-blue-700' },
    { label: 'Artists', value: stats.totalArtists, icon: Headphones, color: 'bg-green-100 text-green-700' },
    { label: 'Genres', value: stats.totalGenres, icon: Heart, color: 'bg-purple-100 text-purple-700' },
    { label: 'Avg BPM', value: stats.avgBpm, icon: Zap, color: 'bg-orange-100 text-orange-700' },
  ];
...
const RecentTracksSection = ({ tracks }: { tracks: CrateTrack[] }) => {
  const { togglePlayPause, playingTrackId, isPlaying, setQueue } = usePlayerStore();
    ...
    <Card variant="elevated">
      ...
      <CardContent className="p-4">
          ...
          {recentTracks.map((track) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium text-sm truncate text-text">{track.title}</div>
        ...
        <div className="mt-4 pt-4 border-t-2 border-black">
          <Link href={`/${useAuthStore.getState().userIdentity?.username}/tracks`}>
          ...
          {actions.map((action) => (
            <div key={action.title}>
              {action.href === '#' ? (
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className={cn('p-2 rounded-base text-white', action.color)}>
...
const DashboardContent = ({ username }: { username: string }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch tracks
        const tracksRes = await fetch('/api/music/tracks', { credentials: 'include' });
        if (tracksRes.ok) {
