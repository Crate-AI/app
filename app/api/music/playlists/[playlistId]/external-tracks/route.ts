...
export async function POST(
  ...
  try {
    ...
    if (!isExternalTrack(track.id)) {
      return NextResponse.json(
        { error: 'This endpoint is only for external tracks. Use the regular tracks endpoint for collection tracks.' },
        { status: 400 },
      );
    }

    // For external tracks, we'll store them in a separate table or handle them differently
    // For now, let's create a temporary track in the tracks table with a special flag
    ...
    const nextPosition = currentTracks.length > 0 ? currentTracks[0].position + 1 : 1;
    ...
    if (insertError) {
      console.error('Error adding external track to playlist:', insertError);
      return NextResponse.json(
