...
import type { SearchParams, SearchResult, SearchResponse } from '@/types/discogs';
...
export async function POST(request: Request) {
  try {
    ...
    const sortedResults = [...response.results].sort((a: SearchResult, b: SearchResult) => {
      const titleA = (a.title || '').toLowerCase();
      const titleB = (b.title || '').toLowerCase();
      
      // Match all terms
      const aMatchesAll = terms.every((term: string) => titleA.includes(term.toLowerCase()));
      const bMatchesAll = terms.every((term: string) => titleB.includes(term.toLowerCase()));
      
      if (aMatchesAll && !bMatchesAll) return -1;
      if (!aMatchesAll && bMatchesAll) return 1;

      // Count matching terms
      const aMatchCount = terms.filter((term: string) => titleA.includes(term.toLowerCase())).length;
      const bMatchCount = terms.filter((term: string) => titleB.includes(term.toLowerCase())).length;
      ...
      const aStartsWithTerm = terms.some((term: string) => titleA.startsWith(term.toLowerCase()));
      const bStartsWithTerm = terms.some((term: string) => titleB.startsWith(term.toLowerCase()));
