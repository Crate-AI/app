            ...
            <TableRow key={`${release.id}-${trackIndex}`}>
              {trackIndex === 0 && (
                <>
                  ...
                  <TableCell rowSpan={release.masterData?.tracklist?.length} className="font-medium">{release.title}</TableCell>
                  <TableCell rowSpan={release.masterData?.tracklist?.length}>{release.year}</TableCell>
                  <TableCell rowSpan={release.masterData?.tracklist?.length}>{release.artists.join(', ')}</TableCell>
                  <TableCell rowSpan={release.masterData?.tracklist?.length}>{release.labels.join(', ')}</TableCell>
                  <TableCell rowSpan={release.masterData?.tracklist?.length}>{release.genres.join(', ')}</TableCell>
                  <TableCell rowSpan={release.masterData?.tracklist?.length}>{release.styles.join(', ')}</TableCell>
                </>
              )}
              <TableCell>
                {track.position} - {track.title} 
                {track.extraartists && (
                  <span> (feat. {track.extraartists.map(artist => artist.name).join(', ')})</span>
                )}
              </TableCell>
              <TableCell className="iframe-container">
                {release?.masterData?.videos && release.masterData.videos.length > trackIndex && (
                  <iframe
                    className="iframe"
