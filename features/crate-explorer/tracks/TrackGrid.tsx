...
import { convertSearchResultToTrack, createTemporaryTrackForPlayback } from '@/lib/utils/track-conversion';
...
const TrackGrid = ({
  ...
  const { playingTrackId, isPlaying, togglePlayPause, isReady, setQueue } = usePlayerStore();
  const [loadingTrackId, setLoadingTrackId] = useState<string | null>(null);
  
  const searchYouTubeForTrack = async (result: DiscogsSearchResult): Promise<string | null> => {
    try {
      ...
      const response = await fetch(`/api/external/youtube/search?q=${encodeURIComponent(searchQuery)}`);
  ...
  const handlePlayToggle = async (result: DiscogsSearchResult) => {
    ...
    try {
      ...
      if (!tempTrack.youtube_video_id) {
        toast.loading('Finding audio for this track...', { id: 'youtube-search' });
        
        const videoId = await searchYouTubeForTrack(result);
        
        if (!videoId) {
          toast.error('No audio found for this track', { id: 'youtube-search' });
        ...
        toast.success('Audio found! Starting playback...', { id: 'youtube-search' });
      }

      // Convert all search results to tracks for queue (some may need YouTube lookup later)
      const searchTracks = items.map(item => createTemporaryTrackForPlayback(item));
      const trackIndex = searchTracks.findIndex(track => track.id === tempTrack.id);
